...
local SELF_PERPETUATING_TRAITS = {
     ...
     "hcp_trait_superstitious", "hcp_trait_prophetic", "hcp_trait_anger", -- "phar_main_trait_respectful",
     -- "phar_main_trait_irreverent",
     -- "hcp_trait_authoritarian",
     -- "hcp_trait_liberal",
     "hcp_trait_fertile", "hcp_trait_barren", "hcp_trait_blind"
}
-- Traits allowed to trigger for any character on any given turn
local RANDOM_TRAITS_LIST = {"hcp_trait_degenerate"}
...
local DRINK_BUILDING_SET = {["phar_main_happiness_type_b"] = true, ["phar_map_bab_province_management_happiness_growth_type_a"] = true}
...
cm:add_first_tick_callback(function()
	...
	if campaign_traits and campaign_traits.config then
		campaign_traits.config.max_num_traits = 30
		out("max_num_traits has been set to " .. campaign_traits.config.max_num_traits)
	else
		out("Failed to find campaign_traits.config")
	end
...
core:add_listener("hcp_character_completed_battle_main", "CharacterCompletedBattle", true, function(context)
     ...
     if enemy_culture ~= character_faction:culture() then
          if CULTURES_TRAIT_LIST[enemy_culture] ~= nil then
               if character:won_battle() then
                    hcp_main_give_trait(character, "hcp_trait_wins_against_" .. CULTURES_TRAIT_LIST[enemy_culture], 20, 10)
                    out("HCP_BATTLE_WINS_AGAINST_" .. tostring(enemy_culture))
               else
                    hcp_main_give_trait(character, "hcp_trait_defeats_against_" .. CULTURES_TRAIT_LIST[enemy_culture], 20, 50)
...
core:add_listener("hcp_character_turn_end_main", "CharacterTurnEnd", true, function(context)
     ...
     ---- SLOTHFUL ----
     ------------------
     if cm:char_is_general_with_army(character) and character:has_region() and character:turns_in_own_regions() >= 10 and character:in_settlement() and
