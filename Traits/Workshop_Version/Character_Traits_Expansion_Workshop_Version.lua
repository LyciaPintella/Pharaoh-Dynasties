    ...
    function(context)
        ...
        if enemy_culture ~= character_faction:culture() then
            if CULTURES_TRAIT_LIST[enemy_culture] ~= nil then
                if character:won_battle() then
                    hcp_main_give_trait(character, "hcp_trait_wins_against_" .. CULTURES_TRAIT_LIST[enemy_culture], 20,
                        10)
                    out("HCP_BATTLE_WINS_AGAINST_" .. tostring(enemy_culture))
                else
                    hcp_main_give_trait(character, "hcp_trait_defeats_against_" .. CULTURES_TRAIT_LIST[enemy_culture], 20,
                        50)
...
core:add_listener(
    ...
    function(context)
            ...
            for i = 0, building_list:num_items() - 1 do
                if building_list:item_at(i):is_null_interface() == false then
                    ...
                    if MILITARY_ADMIN_BUILDINGS_SET[military_admin_superchain] then
                        ...
                        out("HCP_CHARACTER_" ..
                            character:onscreen_name() .. " FOUND MILITARY ADMIN BUILDING: " .. military_admin_superchain)
                    end
                end
            end
        end
        ...
        if cm:char_is_general_with_army(character)
            ...
            out("HCP_CHARACTER_" ..
                character:onscreen_name() .. " IS IN REGION: " .. region:name() .. " CHECKING FOR MANAGEMENT BUILDINGS")
            for i = 0, building_list:num_items() - 1 do
                if building_list:item_at(i):is_null_interface() == false then
